...
# 生成插件zip文件
zip ../bob-plugin-simpletex-v$version.bobplugin *.js *.json *.png
...
# 计算sha256值
sha256=$(shasum -a 256 bob-plugin-simpletex-v$version.bobplugin | awk '{print $1}')
...
jq --arg version "$version" --arg sha256 "$sha256" --argjson timestamp $timestamp '
    if (.versions | map(.version) | index($version)) then
        ...
        .versions = [{"version": $version, "desc": "更新日志","sha256": $sha256, "url": "https://github.com/wakewon/bob-plugin-simpletex/releases/download/v\($version)/bob-plugin-simpletex-v\($version).bobplugin", "minBobVersion": "0.5.0","timestamp": $timestamp}] + .versions
    end
